...
function parseOrdersHtml(html) {
  ...
  while ((rowMatch = rowRegex.exec(tbodyContent)) !== null) {
    ...
    const serviceName = serviceNameMatch ? serviceNameMatch[1].replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim() : '';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (proxyUrl) {
      console.log(`[custom-history] Using proxy: ${maskProxyUrl(proxyUrl)}`);
    } else if (process.env.VERCEL) {
      console.warn('[custom-history] No proxy configured on Vercel — Cloudflare may block requests');
    }
